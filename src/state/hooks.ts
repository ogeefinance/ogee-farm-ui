...
import { Toast, toastTypes } from '@ogeefinance/uikit'
...
export const usePoolFromPid = (ogeeId): Pool => {
  const pool = useSelector((state: State) => state.pools.data.find((p) => p.ogeeId === ogeeId))
  return pool
}
