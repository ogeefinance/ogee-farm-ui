...
import { AutoRenewIcon, Button, Checkbox, Flex, InjectedModalProps, Text } from '@ogeefinance/uikit'
...
const PauseProfilePage: React.FC<PauseProfilePageProps> = ({ onDismiss }) => {
  ...
  const { numberOytToReactivate } = useGetProfileCosts()
  const TranslateString = useI18n()
  const ogeeYieldProfileContract = useProfileContract()
  ...
  const handleDeactivateProfile = () => {
    ogeeYieldProfileContract.methods
      .pauseProfile()
      .send({ from: account })
      .on('sending', () => {
  ...
  return (
    <>
      ...
      <Text as="p" color="textSubtle" mb="24px">
        {TranslateString(999, `Cost to reactivate in future: ${getBalanceNumber(numberOytToReactivate)} OYT`)}
      </Text>
