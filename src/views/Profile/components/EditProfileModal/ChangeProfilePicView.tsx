...
import { Button, InjectedModalProps, Skeleton, Text } from '@ogeefinance/uikit'
...
import { useOgeeYieldRabbits, useProfile as useProfileContract } from 'hooks/useContract'
import { getOgeeYieldProfileAddress, getOgeeYieldRabbitsAddress } from 'utils/addressHelpers'
...
const ChangeProfilePicPage: React.FC<ChangeProfilePicPageProps> = ({ onDismiss }) => {
  ...
  const ogeeYieldRabbitsContract = useOgeeYieldRabbits()
  ...
  const {
    ...
    onApprove: () => {
      return ogeeYieldRabbitsContract.methods.approve(getOgeeYieldProfileAddress(), tokenId).send({ from: account })
    },
    onConfirm: () => {
      if (!profile.isActive) {
        return profileContract.methods.reactivateProfile(getOgeeYieldRabbitsAddress(), tokenId).send({ from: account })
      }

      return profileContract.methods.updateProfile(getOgeeYieldRabbitsAddress(), tokenId).send({ from: account })
    },
    onSuccess: async () => {
      // Re-fetch profile
    ...
    <>
      ...
      {!isLoading && walletNfts.length === 0 && (
        <>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {TranslateString(999, 'Make sure you have a OgeeYield Collectible in your wallet and try again!')}
          </Text>
        </>
      )}
