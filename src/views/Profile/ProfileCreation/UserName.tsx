...
const minimumOytToRegister = new BigNumber(REGISTER_COST).multipliedBy(new BigNumber(10).pow(18))
...
const UserName: React.FC = () => {
  ...
  const { teamId, tokenId, userName, actions, minimumOytRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumOytRequired = useHasOytBalance(minimumOytToRegister)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumOytRequired={minimumOytRequired}
      allowance={allowance}
    />,
    false,
  )
    ...
    <>
      ...
      {!hasMinimumOytRequired && (
        <Text color="failure" mt="16px">
          {TranslateString(1098, `A minimum of ${REGISTER_COST} OYT is required`, { num: REGISTER_COST })}
        </Text>
      )}
    </>
  )
