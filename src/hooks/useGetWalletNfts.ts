...
import { getOgeeYieldRabbitContract } from 'utils/contractHelpers'
import makeBatchRequest from 'utils/makeBatchRequest'

const ogeeYieldRabbitsContract = getOgeeYieldRabbitContract()
...
const useGetWalletNfts = () => {
  ...
  useEffect(() => {
    const fetchNfts = async () => {
      try {
        const balanceOf = await ogeeYieldRabbitsContract.methods.balanceOf(account).call()
        ...
        if (balanceOf > 0) {
          ...
          const getTokenIdAndBunnyId = async (index: number) => {
            try {
              const { tokenOfOwnerByIndex, getBunnyId, tokenURI } = ogeeYieldRabbitsContract.methods
